...
namespace fclrave
{
    ...
    boost::shared_ptr<fcl::BroadPhaseCollisionManager<float>> CreateManagerFromBroadphaseAlgorithm(std::string const &algorithm)
    {
        if (algorithm == "Naive")
        {
            return boost::make_shared<fcl::NaiveCollisionManager<float>>();
        }
        else if (algorithm == "SaP")
        {
            return boost::make_shared<fcl::SaPCollisionManager<float>>();
        }
        else if (algorithm == "SSaP")
        {
            return boost::make_shared<fcl::SSaPCollisionManager<float>>();
        }
        ...
        {
            return boost::make_shared<fcl::IntervalTreeCollisionManager<float>>();
        }
        else if (algorithm == "DynamicAABBTree")
        {
            return boost::make_shared<fcl::DynamicAABBTreeCollisionManager<float>>();
        }
        else if (algorithm == "DynamicAABBTree1")
        {
            boost::shared_ptr<fcl::DynamicAABBTreeCollisionManager<float>> pmanager = boost::make_shared<fcl::DynamicAABBTreeCollisionManager<float>>();
        ...
        {
            return boost::make_shared<fcl::DynamicAABBTreeCollisionManager_Array<float>>();
        }
        else if (algorithm == "DynamicAABBTree1_Array")
        {
            boost::shared_ptr<fcl::DynamicAABBTreeCollisionManager_Array<float>> pmanager = boost::make_shared<fcl::DynamicAABBTreeCollisionManager_Array<float>>();
            pmanager->tree_init_level = 1;
            return pmanager;
        }
    ...
    FCLCollisionChecker::CollisionCallbackData::CollisionCallbackData(boost::shared_ptr<FCLCollisionChecker> pchecker, CollisionReportPtr report, const std::vector<KinBodyConstPtr> &vbodyexcluded, const std::vector<LinkConstPtr> &vlinkexcluded)
        : _pchecker(pchecker), _report(report), _vbodyexcluded(vbodyexcluded), _vlinkexcluded(vlinkexcluded), bselfCollision(false), _bStopChecking(false), _bCollision(false)
    {
    ...
    bool FCLCollisionChecker::CheckCollision(const OpenRAVE::TriMesh &trimesh, KinBodyConstPtr pbody, CollisionReportPtr report)
    {
        ...
        for (size_t ipoint = 0; ipoint < num_points; ++ipoint)
        {
            ...
            _fclPointsCache[ipoint] = fcl::Vector3f(v.x, v.y, v.z);
        }
        ...
        fcl::CollisionObject<float> ctriobj(ctrigeom);
    ...
    bool FCLCollisionChecker::CheckCollision(const OpenRAVE::AABB &ab, const OpenRAVE::Transform &aabbPose, CollisionReportPtr report)
    {
        ...
        CollisionGeometryPtr cboxgeom = make_shared<fcl::Box<float>>(ab.extents.x * 2, ab.extents.y * 2, ab.extents.z * 2);
        cboxgeom->setUserData(nullptr);
        fcl::CollisionObject<float> cboxobj(cboxgeom);

        fcl::Vector3f newPosition = ConvertVectorToFCL(aabbPose * ab.pos);
        fcl::Quaternionf newOrientation = ConvertQuaternionToFCL(aabbPose.rot);
    ...
    bool FCLCollisionChecker::CheckNarrowPhaseCollision(fcl::CollisionObject<float> *o1, fcl::CollisionObject<float> *o2, void *data)
    {
        CollisionCallbackData *pcb = static_cast<CollisionCallbackData *>(data);
        return pcb->_pchecker->CheckNarrowPhaseCollision(o1, o2, pcb);
    }

    bool FCLCollisionChecker::CheckNarrowPhaseCollision(fcl::CollisionObject<float> *o1, fcl::CollisionObject<float> *o2, CollisionCallbackData *pcb)
    {
    ...
    bool FCLCollisionChecker::CheckNarrowPhaseGeomCollision(fcl::CollisionObject<float> *o1, fcl::CollisionObject<float> *o2, void *data)
    {
        CollisionCallbackData *pcb = static_cast<CollisionCallbackData *>(data);
        return pcb->_pchecker->CheckNarrowPhaseGeomCollision(o1, o2, pcb);
    }

    bool FCLCollisionChecker::CheckNarrowPhaseGeomCollision(fcl::CollisionObject<float> *o1, fcl::CollisionObject<float> *o2, CollisionCallbackData *pcb)
    {
        if (pcb->_bStopChecking)
        {
            return true; // don't test anymore
        ...
        else
        {
            ...
            pcb->_request.cached_gjk_guess = fcl::Vector3f(1, 0, 0);
        }
#endif
        ...
        if (numContacts > 0)
        {
            if (!!pcb->_report)
            {
                ...
                if (_options & (OpenRAVE::CO_Contacts | OpenRAVE::CO_AllGeometryContacts))
                {
                    ...
                    {
                        fcl::Contact<float> const &c = pcb->_result.getContact(i);
                        if (bSwapped)
                        {
    ...
    bool FCLCollisionChecker::CheckNarrowPhaseDistance(fcl::CollisionObject<float> *o1, fcl::CollisionObject<float> *o2, void *data, float &dist)
    {
        CollisionCallbackData *pcb = static_cast<CollisionCallbackData *>(data);
        return pcb->_pchecker->CheckNarrowPhaseDistance(o1, o2, pcb, dist);
    }

    bool FCLCollisionChecker::CheckNarrowPhaseDistance(fcl::CollisionObject<float> *o1, fcl::CollisionObject<float> *o2, CollisionCallbackData *pcb, float &dist)
    {
        std::pair<FCLSpace::FCLKinBodyInfo::LinkInfo *, LinkConstPtr> o1info = GetCollisionLink(*o1), o2info = GetCollisionLink(*o2);
    ...
    bool FCLCollisionChecker::CheckNarrowPhaseGeomDistance(fcl::CollisionObject<float> *o1, fcl::CollisionObject<float> *o2, void *data, float &dist)
    {
        CollisionCallbackData *pcb = static_cast<CollisionCallbackData *>(data);
        return pcb->_pchecker->CheckNarrowPhaseGeomDistance(o1, o2, pcb, dist);
    }

    bool FCLCollisionChecker::CheckNarrowPhaseGeomDistance(fcl::CollisionObject<float> *o1, fcl::CollisionObject<float> *o2, CollisionCallbackData *pcb, float &dist)
    {
...
#ifdef NARROW_COLLISION_CACHING
    CollisionPair FCLCollisionChecker::MakeCollisionPair(fcl::CollisionObject<float> *o1, fcl::CollisionObject<float> *o2)
    {
        if (o1 < o2)
        {
    ...
    std::pair<FCLSpace::FCLKinBodyInfo::LinkInfo *, LinkConstPtr> FCLCollisionChecker::GetCollisionLink(const fcl::CollisionObject<float> &collObj)
    {
    ...
    std::pair<FCLSpace::FCLKinBodyInfo::FCLGeometryInfo *, GeometryConstPtr> FCLCollisionChecker::GetCollisionGeometry(const fcl::CollisionObject<float> &collObj)
    {
        const std::shared_ptr<const fcl::CollisionGeometry<float>> &collgeom = collObj.collisionGeometry();
