...
namespace fclrave
{
    ...
    typedef boost::shared_ptr<fcl::CollisionObject<float>> CollisionObjectPtr;
    typedef boost::function<CollisionGeometryPtr(std::vector<fcl::Vector3f> const &points, std::vector<fcl::Triangle> const &triangles)> MeshFactory;
    typedef std::vector<fcl::CollisionObject<float> *> CollisionGroup;
    ...
    inline Vector ConvertVectorFromFCL(fcl::Vector3f const &v)
    {
        return Vector(v[0], v[1], v[2]);
    }

    inline fcl::Vector3f ConvertVectorToFCL(Vector const &v)
    {
        return fcl::Vector3f(v.x, v.y, v.z);
    }
    inline fcl::Quaternionf ConvertQuaternionToFCL(Vector const &v)
    {
        return fcl::Quaternionf(v[0], v[1], v[2], v[3]);
    }

    inline Vector ConvertQuaternionFromFCL(fcl::Quaternionf const &v)
    {
        return Vector(v.w(), v.x(), v.y(), v.z());
    }

    inline fcl::AABB<float> ConvertAABBToFcl(const OpenRAVE::AABB &bv)
    {
