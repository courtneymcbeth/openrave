...
namespace fclrave
{
    ...
    template <class T>
    CollisionGeometryPtr ConvertMeshToFCL(std::vector<fcl::Vector3f> const &points, std::vector<fcl::Triangle> const &triangles)
    {
    ...
    FCLSpace::FCLSpace(EnvironmentBasePtr penv, const std::string &userdatakey)
        : _penv(penv), _userdatakey(userdatakey), _currentpinfo(1, FCLKinBodyInfoPtr()) // initialize with one null pointer, this is a place holder for null pointer so that we can return by reference. env id 0 means invalid so it's consistent with the definition as well
          ,
        ...
        FOREACHC(itlink, pbody->GetLinks())
        {
            ...
            if (pinfo->_geometrygroup.size() > 0 && plink->GetGroupNumGeometries(pinfo->_geometrygroup) >= 0)
            {
                ...
                FOREACH(itgeominfo, vgeometryinfos)
                {
                    ...
                    CollisionObjectPtr pfclcoll = boost::make_shared<fcl::CollisionObject<float>>(pfclgeom);
            ...
            {
                CollisionGeometryPtr pfclgeomBV = std::make_shared<fcl::Box<float>>(enclosingBV.max_ - enclosingBV.min_);
                pfclgeomBV->setUserData(nullptr);
                CollisionObjectPtr pfclcollBV = boost::make_shared<fcl::CollisionObject<float>>(pfclgeomBV);
    ...
    void FCLSpace::SetBVHRepresentation(std::string const &type)
    {
        ...
        if (type == "AABB")
        {
            ...
            _meshFactory = &ConvertMeshToFCL<fcl::AABB<float>>;
        }
        else if (type == "OBB")
        {
            _bvhRepresentation = type;
            _meshFactory = &ConvertMeshToFCL<fcl::OBB<float>>;
        }
        else if (type == "RSS")
        {
            _bvhRepresentation = type;
            _meshFactory = &ConvertMeshToFCL<fcl::RSS<float>>;
        }
        else if (type == "OBBRSS")
        {
            _bvhRepresentation = type;
            _meshFactory = &ConvertMeshToFCL<fcl::OBBRSS<float>>;
        }
        else if (type == "kDOP16")
        {
            _bvhRepresentation = type;
            _meshFactory = &ConvertMeshToFCL<fcl::KDOP<float, 16>>;
        }
        else if (type == "kDOP18")
        {
            _bvhRepresentation = type;
            _meshFactory = &ConvertMeshToFCL<fcl::KDOP<float, 18>>;
        }
        else if (type == "kDOP24")
        {
            _bvhRepresentation = type;
            _meshFactory = &ConvertMeshToFCL<fcl::KDOP<float, 24>>;
        }
        else if (type == "kIOS")
        {
            _bvhRepresentation = type;
            _meshFactory = &ConvertMeshToFCL<fcl::kIOS<float>>;
        }
        else
        {
    ...
    void _AppendFclBoxCollsionObject(const OpenRAVE::Vector &fullExtents, const OpenRAVE::Vector &pos, std::vector<std::shared_ptr<fcl::CollisionObject<float>>> &contents)
    {
        std::shared_ptr<fcl::CollisionGeometry<float>> fclGeom = std::make_shared<fcl::Box<float>>(fullExtents.x, fullExtents.y, fullExtents.z);
        fcl::Transform3f fclTrans = fcl::Transform3f::Identity();
        fclTrans.translation() = fcl::Vector3f(pos.x, pos.y, pos.z);
        contents.emplace_back(std::make_shared<fcl::CollisionObject<float>>(fclGeom, fclTrans));
    }

    /// \brief helper function to initialize fcl::Container
    void _AppendFclBoxCollsionObject(const OpenRAVE::Vector &fullExtents, const OpenRAVE::Transform &trans, std::vector<std::shared_ptr<fcl::CollisionObject<float>>> &contents)
    {
        std::shared_ptr<fcl::CollisionGeometry<float>> fclGeom = std::make_shared<fcl::Box<float>>(fullExtents.x, fullExtents.y, fullExtents.z);
        const fcl::Quaternionf fclQuat = ConvertQuaternionToFCL(trans.rot);
        const fcl::Vector3f fclTranslation = ConvertVectorToFCL(trans.trans);
        fcl::Transform3f fclTrans = fcl::Transform3f::Identity();
        fclTrans.linear() = fclQuat.toRotationMatrix();
        fclTrans.translation() = fclTranslation;
        contents.emplace_back(std::make_shared<fcl::CollisionObject<float>>(fclGeom, fclTrans));
    }
    ...
    CollisionGeometryPtr FCLSpace::_CreateFCLGeomFromGeometryInfo(const KinBody::GeometryInfo &info)
    {
        ...
        case OpenRAVE::GT_Box:
            return std::make_shared<fcl::Box<float>>(info._vGeomData.x * 2.0f, info._vGeomData.y * 2.0f, info._vGeomData.z * 2.0f);

        case OpenRAVE::GT_Sphere:
            return std::make_shared<fcl::Sphere<float>>(info._vGeomData.x);

        case OpenRAVE::GT_Cylinder:
            return std::make_shared<fcl::Cylinder<float>>(info._vGeomData.x, info._vGeomData.y);
        ...
        case OpenRAVE::GT_Container:
        {
            ...
            std::vector<std::shared_ptr<fcl::CollisionObject<float>>> contents;
            // +x wall
            ...
            _AppendFclBoxCollsionObject(Vector(outerextents[0], (outerextents[1] - innerextents[1]) / 2.0, outerextents[2]), Vector(0.0, -(outerextents[1] + innerextents[1]) / 4.0, outerextents[2] / 2.0 + zoffset), contents);
            // bottom
            if (outerextents[2] - innerextents[2] >= 1e-6)
            { // small epsilon error can make thin triangles appear, so test with a reasonable threshold
        ...
        case OpenRAVE::GT_TriMesh:
        {
            ...
            std::vector<fcl::Vector3f> fcl_points(num_points);
            for (size_t ipoint = 0; ipoint < num_points; ++ipoint)
            {
                Vector v = mesh.vertices[ipoint];
                fcl_points[ipoint] = fcl::Vector3f(v.x, v.y, v.z);
            }
    ...
    void FCLSpace::_Synchronize(FCLKinBodyInfo &info, const KinBody &body)
    {
        ...
        if (info.nLastStamp != body.GetUpdateStamp())
        {
            ...
            for (size_t i = 0; i < body.GetLinks().size(); ++i)
            {
                ...
                const fcl::Vector3f newPosition = ConvertVectorToFCL(pose.trans);
                const fcl::Quaternionf newOrientation = ConvertQuaternionToFCL(pose.rot);
                ...
                {
                    fcl::CollisionObject<float> &coll = *pgeom.second;
                    const Transform pose1 = linkTransform * pgeom.first;
                    const fcl::Vector3f newPosition1 = ConvertVectorToFCL(pose1.trans);
                    const fcl::Quaternionf newOrientation1 = ConvertQuaternionToFCL(pose1.rot);
    ...
    void FCLSpace::_ResetCurrentGeometryCallback(boost::weak_ptr<FCLKinBodyInfo> _pinfo)
    {
        ...
        if (0 < bodyIndex && bodyIndex < (int)_currentpinfo.size())
        {
            ...
            if (!!pinfo && pinfo == pcurrentinfo)
            { // pinfo->_geometrygroup.size() == 0 ) {
